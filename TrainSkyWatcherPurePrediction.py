...
def main():
    ...
    for epoch in range(epochs):
        ...
        for (inputs1, response1Cpu), (inputs2, response2Cpu) in zip(
                                            dataMgr.dataResponseGenerator(dataMgr.m_trainingSetIndices, shuffle=True, randomROI=True, useBootStrapping=True),
                                            dataMgr.dataResponseGenerator(dataMgr.m_trainingSetIndices, shuffle=True, randomROI=True, useBootStrapping=True)):
            if epoch % 5 == 0:
        ...
        if not mergeTrainTestData:
            ...
            with torch.no_grad():
                for inputs, responseCpu in dataMgr.dataResponseGenerator(dataMgr.m_validationSetIndices, shuffle=True, randomROI=False, useBootStrapping=True):
