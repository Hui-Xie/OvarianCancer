# Brute force search Lambda for 2 Unet
...
sys.path.append(".")
from SearchLambda2Unet import SearchLambda2Unet
...
def main():
    ...
    # use validation data for searching Lambda
    ...
    if "OCT_Tongren" in hps.dataDir:
        ...
        if hps.numSurfaces == 10:
            surfaceNames = ['ILM', 'RNFL-GCL', 'GCL-IPL', 'IPL-INL', 'INL-OPL', 'OPL-HFL', 'BMEIS', 'IS/OSJ', 'IB_RPE',
                            'OB_RPE']
    ...
    with torch.no_grad():
        ...
        lambdaVecList = []
        lambdaVecList.append(initialLambdaVec)
        meanErrorList = []
        surfErrorList = []
        meanStdList = []
        surfStdList = []
    ...
    for b in range(B):
        ...
        if "OCT_JHU" in hps.dataDir:
            ...
            patientID_Index = os.path.splitext(os.path.basename(testIDs[b]))[
                0]  # e.g. hc01_spectralis_macula_v1_s1_R_19
        ...
        if OutputNumImages >= 3 and hps.existGTLabel:
            ...
            if MarkGTDisorder:
                ...
                if len(errorLocations[0]) > 0:
                    subplot1.scatter(errorLocations[1], testGts[b, errorLocations[0], errorLocations[1]], s=1, c='r',
                                     marker='o')  # red for gt disorder
            if MarkPredictDisorder:
                ...
                if len(errorLocations[0]) > 0:
                    subplot1.scatter(errorLocations[1], testOutputs[b, errorLocations[0], errorLocations[1]], s=1,
                                     c='g', marker='o')  # green for prediction disorder
            subplot1.axis('off')
