...
# directly use softmax in H dimension to get N surface, and then deduce N-1 thickness, but use surface 0 ground truth only.
# add smooth module
...
class ThicknessSubnet_M5(BasicModel):  #
    ''''
    ...
    def forward(self, inputs, gaussianGTs=None, GTs=None, layerGTs=None, riftGTs=None):
        ...
        mu, sigma2 = computeMuVariance(surfaceProb, layerMu=None, layerConf=None)
        B,N,W = mu.shape
        S = mu
        ...
        if self.getStatus() != "test":
            ...
            loss = loss_layer +  loss_ce + loss_thicknessL1
