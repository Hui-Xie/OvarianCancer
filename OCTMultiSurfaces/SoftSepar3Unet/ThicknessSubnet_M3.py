...
# directly mean of (1x1 conv) to get N surface, and then deduce N-1 thickness, but use surface 0 ground truth only.
# add smooth module
...
class ThicknessSubnet_M3(BasicModel):  #
    ''''
    ...
    def __init__(self, hps=None):
        ...
        self.m_surface = nn.Sequential(
            ...
            nn.Conv2d(N//2, hps.numSurfaces, kernel_size=[1,1], stride=[1,1], padding=[0,0]),  # BxnumSurfacexHxW
        )  # output size:Bx(numSurfaces)*H*W
    ...
    def forward(self, inputs, gaussianGTs=None, GTs=None, layerGTs=None, riftGTs=None):
        ...
        xs = self.m_surface(x)  # output size: BxNxHxW, below N is numSurfaces.
        xs = torch.mean(xs, dim=-2, keepdim=False)   # output size: BxNxW
