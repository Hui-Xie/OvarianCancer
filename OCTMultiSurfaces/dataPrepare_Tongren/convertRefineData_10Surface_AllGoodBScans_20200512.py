...
NumSlices = 31  # default for each patient for test and validation set
...
outputDir = "/home/hxie1/data/OCT_Tongren/numpy/10FoldCVForMultiSurfaceNet_10Surfaces_AllGoodBscans"
...
def saveVolumeSurfaceToNumpy(volumesList, goalImageFile, goalSurfaceFile, goalPatientsIDFile):
    ...
    allPatientsImageArray = np.empty((totalNumSlices, H, W), dtype=np.float)
    allPatientsSurfaceArray = np.empty((totalNumSlices, N, W),dtype=np.int)
    patientIDDict = {}
    ...
    for volume in volumesList:
        ...
        if "5363_OD_25453" == patientName:
            surfacesArray = surfacesArray[lowB:highB, :, 103:615]  # left shift 25 pixels for case 5363_OD_25453
        else:
            surfacesArray = surfacesArray[lowB:highB, :, 128:640]
        allPatientsSurfaceArray[s:s+nSlices,:,:] = surfacesArray
