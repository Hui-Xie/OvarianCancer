...
dataSetDir = "/home/hxie1/data/BES_3K/numpy/W512/test"
...
images = np.load(os.path.join(dataSetDir,f"images_{kf:d}.npy"))
NSlice1,H,W = images.shape

with open(os.path.join(dataSetDir,f"patientID_{kf:d}.json")) as json_file:
    patientIDs = json.load(json_file)

k= 71 # random test number
...
plt.savefig(os.path.join(outputDir, patientID_Index + "_Raw.png"), dpi='figure', bbox_inches='tight',pad_inches=0)
