...
from OvarianCancerSeg.trial.ResNeXtBlock import ResNeXtBlock
from framework.SpatialTransformer import SpatialTransformer
from framework.DeformConvBlock import DeformConvBlock
...
class ResNeXtVNet(BasicModel):
    def forward(self, x, halfForward=False):
        ...
        x5 = self.m_down5(x4)
        x5 = x5.view(-1,512)
        x5 = zeroMeanNormalize(x5,dim=1)
        x6 = self.m_bottomFC(x5)
        x6 = zeroMeanNormalize(x6, dim=1)  # this is for output of latent vector
