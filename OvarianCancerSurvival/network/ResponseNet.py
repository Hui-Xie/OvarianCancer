...
class ResponseNet(BasicModel):
    ...
    def forward(self, inputs, GTs=None):
        ...
        residualGT = torch.tensor(GTs['ResidualTumor'] + 1).to(device).view(1,1)   # from [-1,0,1,2] to [0,1,2,3]
        ...
        if GTs['ChemoResponse'] != -100: # -100 ignore index
            chemoGT = torch.tensor(GTs['ChemoResponse']).to(device).view(1,1)  # [0,1]
        ...
        if GTs['Age'] != -100:  # -100 ignore index
            ageGT = torch.tensor(GTs['Age']).to(device).view(1,1)  # range [0,100)
        ...
        P = nn.functional.sigmoid(survivalFeature)  # survival prob
        survivalPredict = torch.argmax(P>= 0.5)  # argmax return the  position of the last maximum.
        survivalLoss = 0.0
        if (GTs['SurvivalMonths'] != -100) and (GTs['Censor'] != -100):
            ...
            if 1 == GTs['Censor']:
                ...
                expz_i = -torch.floatTensor(list(range(0,R))).to(device)
