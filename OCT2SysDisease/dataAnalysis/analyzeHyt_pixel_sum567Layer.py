...
# for 9x15x12 or 9x31x25 images.
# sum the 5th, 6th,7th, 8th thickness layers (index starting from 0) together for analysis.
...
def retrieveImageData_label(mode, hps):
    ...
    :param hps:
    :return: volumes: volumes of all patient in this data: Nx1xHxW; where 1 is summed channels of 5,6,7,8
    ...
    # get channel 5,6,7 and sum together.
    volumes = np.sum(volumes[:,5:-1,:,:], axis=1, keepdims=True) # size: Nx1xHxW
...
def main():
    ...
    # pValues* has size: 1xHxW

    # mask[c,h,w]=1 means the t-test at location pvalue(c,h,w) < 0.05 crossing training, validation and test
    ...
    maskPath = os.path.join(hps.outputDir,f"{maskname}_sum567Layers_{C}x{H}x{W}.npy")
