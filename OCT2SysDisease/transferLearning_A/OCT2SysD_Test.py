...
from Retina3D_HBP_Net import Retina3D_HBP_Net
...
def main():
    ...
    with torch.no_grad():
        ...
        net.setStatus("test")
        for batchData in data.DataLoader(testData, batch_size=1, shuffle=False, num_workers=0):
            inputs = batchData['images'].squeeze(dim=0)  # B,C,H,W, where squeeze for 10-crops test
    ...
    Acc_TPR_TNR_Sum = compute_Acc_TPR_TNR_Sum_WithProb(allTestGTs, allTestOutput,threshold)
    ...
    with open(os.path.join(hps.outputDir, f"testOutput_{timeStr}.txt"), "w") as file:
        ...
        Acc_TPR_TNR_Sum2 = compute_Acc_TPR_TNR_Sum_WithProb(allTestGTs, allTestOutput, 0.5)
