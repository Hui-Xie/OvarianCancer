...
def main():
    ...
    print("sum layer 5,6,7, and 8 together")
    enfaceVolume = np.sum(enfaceVolume[5:, :,:],axis=0, keepdims=True)
    ...
    for (volume, outputFilename) in volume_name_list:
        ...
        if W >=9*H:
            ...
            rowSubplot = 1  # for sum layer 5,6,7,8 image
            colSubplot = 1
        else:
        ...
        outputFilePath = os.path.join(outputDir, outputFilename+ f"_sum5678_{N}x{H}x{W}.png")
